...
def get_user_json_directory():
    ...
    app_mode = input(
        "\nEnter a Mode for application (excel or json or zipped or excel-json):  "
    ).lower()
    ...
    while not app_mode_validation:
        if app_mode not in allowed_modes:
            print(
                "\nPlease Enter a Valid Mode - only 'excel' or 'json' or 'zipped' or 'excel-json' are allowed\n"
            )
            app_mode = input(
                "\nEnter a Mode for application (excel or json or zipped or excel-json):  "
            ).lower()
...
def write_basic_data(path_to_json):
    global basic_data, app_mode, create_excel_dir_modes
